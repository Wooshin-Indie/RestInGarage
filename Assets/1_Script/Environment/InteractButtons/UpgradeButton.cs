...
{
	public class UpgradeButton : InteractButton
	{
		private void Update()
		{
			...
			if (isSomeoneDetected && BuildingNetworkManager.Instance.IsAbleToUpgrade())
			{
				elapsedTime.Value += Time.deltaTime;
				if (elapsedTime.Value > maxTime)
				{
					BuildingNetworkManager.Instance.UpgradePropsServerRPC();
