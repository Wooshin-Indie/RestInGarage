...
#define INIT_UTSLIGHT(utslight) \
            ...
            VertexOutput vert (VertexInput v) {
                ...
                // 흔들림 애니메이션 함수를 호출하여 정점 위치를 업데이트합니다
                positionOS = ApplyWaveAnimation(positionOS, v.texcoord0);
...
//
                ...
                // 수정
                // v.vertex 대신 수정된 positionOS 사용
                o.posWorld = mul(GetObjectToWorldMatrix(), float4(positionOS, 1.0)); // v.vertex를 positionOS로 변경
                o.pos = UnityObjectToClipPos(float4(positionOS, 1.0)); // v.vertex를 positionOS로 변경
                ...
                // 수정
                // TransformObjectToWorld에도 수정된 positionOS 사용
                float3 positionWS = TransformObjectToWorld(positionOS.xyz); // v.vertex.xyz를 positionOS.xyz로 변경
...
#include "WavingToonBodyShadingGradeMap.hlsl"

#else //#if defined(_SHADINGGRADEMAP)

#include "WavingToonBodyDoubleShadeWithFeather.hlsl"
